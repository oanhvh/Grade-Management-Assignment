...
public class AssessmentController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            assessments = dao.getAssessmentsByCourseId(courseId);
            
            Course course = cdao.getCourseById(courseId);
            
            request.setAttribute("assessments",assessments);
            request.setAttribute("course",course);
            request.setAttribute("total",assessments.size());
            request.getRequestDispatcher("assessments.jsp").forward(request, response);
        } catch (SQLException ex) {
            Logger.getLogger(AssessmentController.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
